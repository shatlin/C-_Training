...
function LoadPage()
{
	...
	try{mainSection.scrollTop = scrollPos;}
	 ...
	 //Change Type: Added Code to hide seealso anchor.
 // Change Date: 2/17/2005
 ///////////////////////
 Hide_SeeAlsoAnchorTop()
 ///////////////////////

 //Change Type: Added Code for expand collapse anywhere.
 // Change Date: 2/18/2005
 ///////////////////////
 CreateExpandCollapse("h1");
 CreateExpandCollapse("h4");
 setCollapse();
 ///////////////////////
 
 //Change Type: Added Code for glossary page.
 // Change Date: 3/4/2005
 ///////////////////////
 if(document.title.indexOf("Glossary") > -1)
 {
  document.glossary = new Glossary();
  document.glossary.LoadGlossary();
 }
 ///////////////////////
}
...
function ExpandCollapseAll(imageItem)
{
	...
	if(ItemCollapsed(imageItem.id) == true)
	{
        ...
        imageItem.parentNode.innerHTML = imageItem.outerHTML + "Collapse All";
		RemoveCollapsedItem(imageItem.id);
		for(i = 0; i < imgElements.length; ++i)
		{
	...
	else
	{
        ...
        imageItem.parentNode.innerHTML = imageItem.outerHTML + "Expand All";
		AddCollapsedItem(imageItem.id);
		for(i = 0; i < imgElements.length; ++i)
		{
...
function SetCollapseAll()
{
	...
	if(allCollapsed)
	{
		...
		imageElement.parentNode.innerHTML = imageElement.outerHTML + "Expand All";
		AddCollapsedItem(imageElement.id);
	}
	else
	{
		imageElement.src = collapseAllImage.src;
		imageElement.parentNode.innerHTML = imageElement.outerHTML + "Collapse All";
...
// Change Date: 2/17/2005
///////////////////////
function Hide_SeeAlsoAnchorTop()
{
  if(headerTableRow3 != null && document.body.innerHTML.indexOf("{{expando}}")> -1)
  {
   var atag = headerTableRow3.children.item(0);
   
   atag.innerHTML="<span onclick=\"ExpandCollapseAll(toggleAllImage)\" style=\"cursor:default;\"><img ID=\"toggleAllImage\" class=\"toggleAll\" src=\""+expandAllImage.src+"\"></img>Expand All</span>";

   atag.children.item(0).style.cursor = "hand";
  }
  else
   headerTableRow3.style.display = "none";
}
///////////////////////
...
// Change Date: 2/18/2005
///////////////////////
 function CreateExpandCollapse(eventtag)
 {
  var tagElements = document.all.tags(eventtag);
  var searchstring = "{{expando}}";
  var imgid = "sectionToggle";
  var j=0;
 
  for(i=0;i<tagElements.length;i++)
  {
   if(tagElements[i].innerHTML.indexOf(searchstring) > -1)
   {
    var titlespan = tagElements[i].innerHTML.replace(searchstring,"");
    var imgidrepace = imgid+tagElements[i].nodeName;
    tagElements[i].innerHTML = "<span onclick=\"ExpandCollapse("+imgidrepace+j+")\"><img id=\""+imgidrepace+j+"\" class=\"toggle\" name=\"toggleSwitch\" src=\""+collapseImage.src+"\" />"+titlespan+"</span>";
    tagElements[i].style.cursor = "hand";       
    j++
   }
  }
 }
 ...
 function setCollapse()
 {
  ...
  for(i=0;i<imgElements.length;i++)
  {
   if(imgElements[i].id.indexOf("sectionToggle") > -1)
   ExpandCollapse(imgElements[i]);
  }
 }
///////////////////////

//Change Type: Added Code for glossary.
// Change Date: 3/4/2005
///////////////////////

/*************************************************************************
 * BIApps UA Glossary Class
...
function Glossary()
{
 this.glossaryTermNameArray = new Array(0);
 this.glossDefinitionArray  = new Array(0);
}
...
Glossary.prototype.LoadGlossary = _LoadGlossary;
function _LoadGlossary()
{
 var i,j, term, def;
 var heading = document.body.all.tags("h3");

 for(i = 0; i < heading.length; i++)
 {
  var tablenode = heading[i].nextSibling;
  var paras = tablenode.getElementsByTagName("td");
  for(j = 0; j < paras.length; j++)
  {
   term = paras[j++].innerText;
   def = paras[j].innerHTML;
   this.glossaryTermNameArray.push(term);
   this.glossDefinitionArray[term]=def;
  }//end for j
 }//end for i
 mainBody.innerHTML = "<div id='terms'/></div>";
 headerTableRow3.firstChild.innerHTML = this.GetLetterNavigator();
 headerTableRow3.style.display = "";
 this.RenderGlossary("ALL",terms);
}//end funciton

Glossary.prototype.RenderGlossary = _RenderGlossary;
function _RenderGlossary(termSet, spanTag)
{
  var terms = "";
  ...
  this.glossaryTermNameArray.sort(this.SortArray);

  for(i = 0;i < this.glossaryTermNameArray.length;i++)
  {
    termName = this.glossaryTermNameArray[i];
    firstLetter = termName.substr(0,1).toUpperCase();

    if(termSet == "ALL")
    {
      if(lastLetter != firstLetter)
       terms  += "<h1 class='heading'><font color='#003399'>"+firstLetter+"</font></h1><HR SIZE='1' />";

      terms += this.CreateGlossEntry(termName,this.glossDefinitionArray[termName]);

      lastLetter = firstLetter;

    }
    if(termSet == firstLetter)
    {
      if(lastLetter == "")
       terms  += "<h1 class='heading'><font color='#003399'>"+firstLetter+"</font></h1><HR SIZE='1' />";
 ...
 spanTag.innerHTML = terms;
 spanTag.scrollIntoView(true);
 window.scrollTo(0,0);
}
Glossary.prototype.GetLetterNavigator = _GetLetterNavigator;
function _GetLetterNavigator()
{
 var firstCharacter = "";
 var array = new Array();

 var alphaArrary = new Array("a","b","c","d","e","f","g","h","i","j","k","l","m","n","o","p","q","r","s","t","u","v","w","x","y","z");
 ...
 nodeList =  this.glossaryTermNameArray;

 for (var i = 0;i < nodeList.length;i++)
 {
  firstCharacter = nodeList[i].substr(0,1).toUpperCase();
  if(link.indexOf(firstCharacter) == -1)
 ...
 link = "";
 link += "<a onclick=\"document.glossary.RenderGlossary('ALL', terms)\" onmouseover=\"this.style.cursor='hand'\"><u>All</u></a>  ";

 for(i = 0;i < alphaArrary.length;i++)
 {
  if(array.toString().indexOf(alphaArrary[i].toUpperCase()) != -1)
  {
   link += "<a onclick=\"document.glossary.RenderGlossary('"+alphaArrary[i].toUpperCase()+"',terms)\" onmouseover=\"this.style.cursor='hand'\"><u>";
   link += alphaArrary[i].toUpperCase();
   link += "</u></a>   ";
  }
  else
  {
...
Glossary.prototype.CreateGlossEntry = _CreateGlossEntry;
function _CreateGlossEntry(term, def)
{
  var glossaryTerms = "";
      glossaryTerms += "<P><B>";
      glossaryTerms += term;
      glossaryTerms += "</B></P>";
      glossaryTerms += "<BLOCKQUOTE>";
      glossaryTerms += def;
      glossaryTerms += "</BLOCKQUOTE>";
  return glossaryTerms;
}
Glossary.prototype.SortArray = _SortArray;
function _SortArray(x, y)
{
